...
describe('buildSprintReportMarkdown', () => {
  it('generates a portable markdown report with health, risks and warmups', () => {
    ...
    expect(markdown).toContain('下一轮训练：开始下一轮训练')
    ...
    expect(markdown).toContain('## 真实面试押题')
    expect(markdown).toContain('个人押题队列')
    expect(markdown).toContain('压力信号：薄弱题、模拟 60 分')
    expect(markdown).toContain('/practice?queue=1&from=experience-playbook')
    expect(markdown).toContain('真实面试押题：开始押题练习')
    expect(markdown).toContain('## 岗位能力地图')
    expect(markdown).toContain('Java 后端冲刺路线｜Java 后端')
    expect(markdown).toContain('准备度 62 分')
    expect(markdown).toContain('薄弱 1 道')
    expect(markdown).toContain('/practice?queue=1,3&from=ability-gap')
    ...
    expect(markdown).toContain('/practice?queue=1&from=interview-retrospective')
    ...
    expect(markdown).toContain('来源：评分影响')
    expect(markdown).toContain('行动：重答补强，入口：/practice?queue=1')
    expect(markdown).toContain('## 下一步行动')
    expect(markdown).toContain('下一轮训练：开始下一轮训练')
    expect(markdown).toContain('能力地图：训练 Java 后端冲刺路线 - 先补 2 道未掌握题，把岗位能力短板拉回学习中。（/practice?queue=1,3&from=ability-gap）')
  })
  ...
  it('keeps empty reports actionable instead of exporting a blank file', () => {
    ...
    expect(markdown).toContain('下一轮训练：先做一次模拟面试')
    expect(markdown).toContain('## 今日作战简报')
