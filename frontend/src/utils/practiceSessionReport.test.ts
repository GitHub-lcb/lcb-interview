...
describe('buildPracticeSessionReport', () => {
  ...
  it('exports training contract for the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
  ...
  it('exports schedule checklist for the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 训练日程打卡清单')
    expect(markdown).toContain('完成口径')
    expect(markdown).toContain('证据模板')
    expect(markdown).toContain('复盘问题')
    expect(markdown).toContain('主行动：')
  })

  it('exports training receipt template for the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('训练目标')
    expect(markdown).toContain('完成证据')
    expect(markdown).toContain('阻断项')
  ...
  it('exports advance gate before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 下一轮准入闸门')
    expect(markdown).toContain('暂缓进入下一轮')
    expect(markdown).toContain('目标清晰')
    expect(markdown).toContain('证据可查')
    expect(markdown).toContain('主行动：')
  })

  it('exports launch packet before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 下一轮启动包')
    expect(markdown).toContain('回修启动包')
    expect(markdown).toContain('打开启动入口')
    expect(markdown).toContain('完成口径')
    expect(markdown).toContain('主行动：')
  })

  it('exports launch checklist before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 启动执行清单')
    expect(markdown).toContain('证据模板')
    expect(markdown).toContain('复盘问题')
    expect(markdown).toContain('完成口径')
    expect(markdown).toContain('主行动：')
  ...
  it('exports first question rehearsal before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('回修首题预演')
    expect(markdown).toContain('开场提示')
    expect(markdown).toContain('通过信号')
    expect(markdown).toContain('证据要求')
    expect(markdown).toContain('主行动：')
  })
  ...
  it('exports first question receipt template before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题回执模板')
    expect(markdown).toContain('回修首题回执')
    expect(markdown).toContain('本次动作')
    expect(markdown).toContain('评分证据')
    expect(markdown).toContain('是否达标')
    expect(markdown).toContain('主行动：')
  })

  it('exports first question receipt acceptance before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题回执验收卡')
    expect(markdown).toContain('回修回执验收卡')
    expect(markdown).toContain('动作明确')
    expect(markdown).toContain('证据可查')
    expect(markdown).toContain('未通过补救')
    expect(markdown).toContain('主行动：')
  })

  it('exports first question release gate before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题放行门禁')
    expect(markdown).toContain('首题暂缓放行')
    expect(markdown).toContain('预演动作')
    expect(markdown).toContain('验收口径')
    expect(markdown).toContain('回执证据')
    expect(markdown).toContain('放行结论')
    expect(markdown).toContain('主行动：')
    expect(markdown.indexOf('## 首题放行门禁')).toBeGreaterThan(markdown.indexOf('## 首题回执验收卡'))
    expect(markdown.indexOf('## 首题放行门禁')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question review template before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复盘模板')
    expect(markdown).toContain('回修首题复盘模板')
    expect(markdown).toContain('评分变化')
    expect(markdown).toContain('证据变化')
    expect(markdown).toContain('阻断变化')
    expect(markdown).toContain('下一题动作')
    expect(markdown).toContain('验收规则：')
    expect(markdown.indexOf('## 首题复盘模板')).toBeGreaterThan(markdown.indexOf('## 首题放行门禁'))
    expect(markdown.indexOf('## 首题复盘模板')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question review acceptance before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复盘验收卡')
    expect(markdown).toContain('回修复盘验收卡')
    expect(markdown).toContain('评分可比')
    expect(markdown).toContain('证据可追溯')
    expect(markdown).toContain('阻断可判定')
    expect(markdown).toContain('下一题可执行')
    expect(markdown).toContain('未通过补救')
    expect(markdown.indexOf('## 首题复盘验收卡')).toBeGreaterThan(markdown.indexOf('## 首题复盘模板'))
    expect(markdown.indexOf('## 首题复盘验收卡')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question review archive before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复盘归档包')
    expect(markdown).toContain('回修复盘归档包')
    expect(markdown).toContain('分数快照')
    expect(markdown).toContain('证据归档')
    expect(markdown).toContain('阻断结论')
    expect(markdown).toContain('下一题种子')
    expect(markdown).toContain('下一轮用途')
    expect(markdown).toContain('丢失风险')
    expect(markdown.indexOf('## 首题复盘归档包')).toBeGreaterThan(markdown.indexOf('## 首题复盘验收卡'))
    expect(markdown.indexOf('## 首题复盘归档包')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question archive reuse checklist before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('回修归档复用清单')
    expect(markdown).toContain('读分数')
    expect(markdown).toContain('带证据')
    expect(markdown).toContain('认阻断')
    expect(markdown).toContain('开下一题')
    expect(markdown).toContain('开场提示')
    expect(markdown).toContain('失败回退')
    expect(markdown.indexOf('## 首题归档复用清单')).toBeGreaterThan(markdown.indexOf('## 首题复盘归档包'))
  ...
  it('exports first question reuse receipt acceptance before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用回执验收卡')
    expect(markdown).toContain('回修复用回执待验收')
    expect(markdown).toContain('分数对齐')
    expect(markdown).toContain('证据引用')
    expect(markdown).toContain('阻断判断')
    expect(markdown).toContain('下一题接续')
    expect(markdown).toContain('通过信号')
    expect(markdown).toContain('补救动作')
    expect(markdown.indexOf('## 首题复用回执验收卡')).toBeGreaterThan(markdown.indexOf('## 首题复用回执模板'))
    expect(markdown.indexOf('## 首题复用回执验收卡')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse release gate before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用放行门禁')
    expect(markdown).toContain('复用暂缓放行')
    expect(markdown).toContain('分数放行')
    expect(markdown).toContain('证据放行')
    expect(markdown).toContain('阻断放行')
    expect(markdown).toContain('下一题放行')
    expect(markdown).toContain('放行检查')
    expect(markdown).toContain('处理动作')
    expect(markdown.indexOf('## 首题复用放行门禁')).toBeGreaterThan(markdown.indexOf('## 首题复用回执验收卡'))
    expect(markdown.indexOf('## 首题复用放行门禁')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review template before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘模板')
    expect(markdown).toContain('回修复用复盘模板')
    expect(markdown).toContain('分数回看')
    expect(markdown).toContain('证据命中')
    expect(markdown).toContain('阻断回收')
    expect(markdown).toContain('下一题回流')
    expect(markdown).toContain('填写提示')
    expect(markdown).toContain('验收规则')
    expect(markdown.indexOf('## 首题复用复盘模板')).toBeGreaterThan(markdown.indexOf('## 首题复用放行门禁'))
    expect(markdown.indexOf('## 首题复用复盘模板')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review acceptance before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘验收卡')
    expect(markdown).toContain('回修复用复盘待验收')
    expect(markdown).toContain('分数验收')
    expect(markdown).toContain('证据验收')
    expect(markdown).toContain('阻断验收')
    expect(markdown).toContain('回流验收')
    expect(markdown).toContain('通过信号')
    expect(markdown).toContain('补救动作')
    expect(markdown.indexOf('## 首题复用复盘验收卡')).toBeGreaterThan(markdown.indexOf('## 首题复用复盘模板'))
    expect(markdown.indexOf('## 首题复用复盘验收卡')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review archive before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘归档包')
    expect(markdown).toContain('回修复用复盘归档包')
    expect(markdown).toContain('复用分数快照')
    expect(markdown).toContain('复用证据归档')
    expect(markdown).toContain('阻断回收结论')
    expect(markdown).toContain('下一轮回流种子')
    expect(markdown).toContain('归档内容')
    expect(markdown).toContain('下一轮用途')
    expect(markdown.indexOf('## 首题复用复盘归档包')).toBeGreaterThan(markdown.indexOf('## 首题复用复盘验收卡'))
    expect(markdown.indexOf('## 首题复用复盘归档包')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review handoff before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘回流清单')
    expect(markdown).toContain('回修复用复盘回流清单')
    expect(markdown).toContain('分数回流')
    expect(markdown).toContain('证据带入')
    expect(markdown).toContain('阻断处置')
    expect(markdown).toContain('下一题启动')
    expect(markdown).toContain('开场提示')
    expect(markdown).toContain('回退动作')
    expect(markdown.indexOf('## 首题复用复盘回流清单')).toBeGreaterThan(markdown.indexOf('## 首题复用复盘归档包'))
    expect(markdown.indexOf('## 首题复用复盘回流清单')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review handoff acceptance before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘回流验收卡')
    expect(markdown).toContain('回修复用复盘回流待验收')
    expect(markdown).toContain('分数可核验')
    expect(markdown).toContain('证据可带入')
    expect(markdown).toContain('阻断可处置')
    expect(markdown).toContain('入口可执行')
    expect(markdown).toContain('通过信号')
    expect(markdown).toContain('补救动作')
    expect(markdown.indexOf('## 首题复用复盘回流验收卡')).toBeGreaterThan(markdown.indexOf('## 首题复用复盘回流清单'))
    expect(markdown.indexOf('## 首题复用复盘回流验收卡')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review handoff release gate before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘回流放行门禁')
    expect(markdown).toContain('回流暂缓放行')
    expect(markdown).toContain('分数放行')
    expect(markdown).toContain('证据放行')
    expect(markdown).toContain('阻断放行')
    expect(markdown).toContain('入口放行')
    expect(markdown).toContain('放行规则')
    expect(markdown).toContain('处理动作')
    expect(markdown).toContain('未放行')
    expect(markdown.indexOf('## 首题复用复盘回流放行门禁')).toBeGreaterThan(markdown.indexOf('## 首题复用复盘回流验收卡'))
    expect(markdown.indexOf('## 首题复用复盘回流放行门禁')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })

  it('exports first question reuse review handoff release receipt before the next session round', () => {
    const markdown = buildPracticeSessionReportMarkdown(
      [question(1), question(2)],
      progress({
        interviewAttempts: {
    ...
    expect(markdown).toContain('## 首题复用复盘回流放行回执')
    expect(markdown).toContain('回流回执待修复')
    expect(markdown).toContain('放行结论')
    expect(markdown).toContain('随身证据')
    expect(markdown).toContain('阻断处理')
    expect(markdown).toContain('下一轮入口')
    expect(markdown).toContain('交接证据')
    expect(markdown).toContain('下一步动作')
    expect(markdown).toContain('未交接')
    expect(markdown.indexOf('## 首题复用复盘回流放行回执')).toBeGreaterThan(markdown.indexOf('## 首题复用复盘回流放行门禁'))
    expect(markdown.indexOf('## 首题复用复盘回流放行回执')).toBeLessThan(markdown.indexOf('## 下一轮训练建议'))
  })
  ...
  it('keeps empty session markdown actionable', () => {
    ...
    expect(markdown).toContain('## 下一轮准入闸门')
    expect(markdown).toContain('等待建立准入样本')
    expect(markdown).toContain('## 下一轮启动包')
    expect(markdown).toContain('等待建立启动样本')
    expect(markdown).toContain('## 启动执行清单')
    ...
    expect(markdown).toContain('## 首题回执模板')
    expect(markdown).toContain('等待首题回执')
    expect(markdown).toContain('## 首题回执验收卡')
    expect(markdown).toContain('等待验收首题回执')
    expect(markdown).toContain('## 首题放行门禁')
    expect(markdown).toContain('等待首题放行裁决')
    expect(markdown).toContain('## 首题复盘模板')
    expect(markdown).toContain('等待首题复盘模板')
    expect(markdown).toContain('## 首题复盘验收卡')
    expect(markdown).toContain('等待验收首题复盘')
    expect(markdown).toContain('## 首题复盘归档包')
    ...
    expect(markdown).toContain('## 首题复用回执验收卡')
    expect(markdown).toContain('等待验收首题复用回执')
    expect(markdown).toContain('## 首题复用放行门禁')
    expect(markdown).toContain('等待首题复用放行裁决')
    expect(markdown).toContain('## 首题复用复盘模板')
    expect(markdown).toContain('等待首题复用复盘模板')
    expect(markdown).toContain('## 首题复用复盘验收卡')
    expect(markdown).toContain('等待验收首题复用复盘')
    expect(markdown).toContain('## 首题复用复盘归档包')
    expect(markdown).toContain('等待首题复用复盘归档')
    expect(markdown).toContain('## 首题复用复盘回流清单')
    expect(markdown).toContain('等待首题复用复盘回流')
    expect(markdown).toContain('## 首题复用复盘回流验收卡')
    expect(markdown).toContain('等待验收首题复用复盘回流')
    expect(markdown).toContain('## 首题复用复盘回流放行门禁')
    expect(markdown).toContain('等待首题复用复盘回流放行裁决')
    expect(markdown).toContain('## 首题复用复盘回流放行回执')
    expect(markdown).toContain('等待首题复用复盘回流放行回执')
    expect(markdown).toContain('## 下一轮训练建议')
