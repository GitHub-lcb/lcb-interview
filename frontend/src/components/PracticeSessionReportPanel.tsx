...
import {
  ...
  buildPracticeSessionFirstQuestionArchiveReuse,
...
export default function PracticeSessionReportPanel({
  ...
  const sessionFirstQuestionReuseReceiptAcceptance = useMemo(
    () => buildPracticeSessionFirstQuestionReuseReceiptAcceptance(queue, progress, progress.updatedAt),
    [progress, queue],
  )
  const sessionFirstQuestionReuseReleaseGate = useMemo(
    () => buildPracticeSessionFirstQuestionReuseReleaseGate(queue, progress, progress.updatedAt),
    [progress, queue],
  )
  ...
  return (
    <section className={`practice-session-report-panel level-${report.level}`} aria-label="本轮模拟面试战报">
      ...
      <div
        className={`practice-session-report-first-receipt-acceptance status-${sessionFirstQuestionReceiptAcceptance.status}`}
        aria-label="首题回执验收卡"
      >
        <div className="practice-session-report-first-receipt-acceptance-head">
          <div>
            <span>首题回执验收卡</span>
            <strong>{sessionFirstQuestionReceiptAcceptance.title}</strong>
            <small>{sessionFirstQuestionReceiptAcceptance.summary}</small>
          </div>
        </div>
        {sessionFirstQuestionReceiptAcceptance.items.length === 0 ? (
          <p>等待验收首题回执。先生成首题回执模板后，系统会给出验收口径。</p>
        ) : (
          <div className="practice-session-report-first-receipt-acceptance-list">
            {sessionFirstQuestionReceiptAcceptance.items.map(item => (
              <article key={item.id}>
                <div>
                  <span>{item.label}</span>
                  <strong>{item.target}</strong>
        ...
        <Button
          size="small"
          type="primary"
          danger={sessionFirstQuestionReceiptAcceptance.status === 'repair'}
          icon={sessionFirstQuestionReceiptAcceptance.status === 'repair' ? <ReloadOutlined /> : <CheckCircleOutlined />}
      ...
      <div
        className={`practice-session-report-first-archive-reuse status-${sessionFirstQuestionArchiveReuse.status}`}
        aria-label="首题归档复用清单"
      >
        <div className="practice-session-report-first-archive-reuse-head">
          <div>
            <span>首题归档复用清单</span>
            <strong>{sessionFirstQuestionArchiveReuse.title}</strong>
            <small>{sessionFirstQuestionArchiveReuse.summary}</small>
          </div>
        </div>
        {sessionFirstQuestionArchiveReuse.items.length === 0 ? (
          <p>等待首题归档复用。先生成首题复盘归档包后，系统会给出复用动作。</p>
        ) : (
          <div className="practice-session-report-first-archive-reuse-list">
            {sessionFirstQuestionArchiveReuse.items.map(item => (
              <article key={item.id}>
                <div>
                  <span>{item.label}</span>
        ...
        <Button
          size="small"
          type="primary"
          danger={sessionFirstQuestionArchiveReuse.status === 'repair'}
          icon={sessionFirstQuestionArchiveReuse.status === 'repair' ? <ReloadOutlined /> : <CheckCircleOutlined />}
      ...
      <div
        className={`practice-session-report-first-reuse-acceptance status-${sessionFirstQuestionReuseReceiptAcceptance.status}`}
        aria-label="首题复用回执验收卡"
      >
        <div className="practice-session-report-first-reuse-acceptance-head">
          <div>
            <span>首题复用回执验收卡</span>
            <strong>{sessionFirstQuestionReuseReceiptAcceptance.title}</strong>
            <small>{sessionFirstQuestionReuseReceiptAcceptance.summary}</small>
          </div>
        </div>
        {sessionFirstQuestionReuseReceiptAcceptance.items.length === 0 ? (
          <p>等待验收首题复用回执。先生成首题复用回执模板后，系统会给出验收口径。</p>
        ) : (
          <div className="practice-session-report-first-reuse-acceptance-list">
            {sessionFirstQuestionReuseReceiptAcceptance.items.map(item => (
              <article key={item.id}>
                <div>
                  <span>{item.label}</span>
                  <strong>{item.target}</strong>
        ...
        <Button
          size="small"
          type="primary"
          danger={sessionFirstQuestionReuseReceiptAcceptance.status === 'repair'}
          icon={sessionFirstQuestionReuseReceiptAcceptance.status === 'repair' ? <ReloadOutlined /> : <CheckCircleOutlined />}
          onClick={() => onNavigate(sessionFirstQuestionReuseReceiptAcceptance.primaryAction.to)}
        >
          {sessionFirstQuestionReuseReceiptAcceptance.primaryAction.label}
        </Button>
      </div>

      <div
        className={`practice-session-report-first-reuse-release status-${sessionFirstQuestionReuseReleaseGate.status}`}
        aria-label="首题复用放行门禁"
      >
        <div className="practice-session-report-first-reuse-release-head">
          <div>
            <span>首题复用放行门禁</span>
            <strong>{sessionFirstQuestionReuseReleaseGate.title}</strong>
            <small>{sessionFirstQuestionReuseReleaseGate.summary}</small>
          </div>
        </div>
        {sessionFirstQuestionReuseReleaseGate.items.length === 0 ? (
          <p>等待首题复用放行裁决。先完成首题复用回执验收后，系统会判断是否进入下一轮。</p>
        ) : (
          <div className="practice-session-report-first-reuse-release-list">
            {sessionFirstQuestionReuseReleaseGate.items.map(item => (
              <article key={item.id} className={`state-${item.state}`}>
                <div>
                  <span>{item.label}</span>
                  <strong>{item.evidence}</strong>
        ...
        <Button
          size="small"
          type="primary"
          danger={sessionFirstQuestionReuseReleaseGate.status === 'blocked'}
          icon={sessionFirstQuestionReuseReleaseGate.status === 'blocked' ? <WarningOutlined /> : <ArrowRightOutlined />}
          onClick={() => onNavigate(sessionFirstQuestionReuseReleaseGate.primaryAction.to)}
        >
          {sessionFirstQuestionReuseReleaseGate.primaryAction.label}
        </Button>
      </div>

      <div
        className={`practice-session-report-first-reuse-review status-${sessionFirstQuestionReuseReviewTemplate.status}`}
        aria-label="首题复用复盘模板"
      >
        <div className="practice-session-report-first-reuse-review-head">
          <div>
            <span>首题复用复盘模板</span>
            <strong>{sessionFirstQuestionReuseReviewTemplate.title}</strong>
            <small>{sessionFirstQuestionReuseReviewTemplate.summary}</small>
          </div>
        </div>
        {sessionFirstQuestionReuseReviewTemplate.items.length === 0 ? (
          <p>等待首题复用复盘模板。先完成首题复用放行门禁后，系统会给出复盘字段。</p>
        ) : (
          <div className="practice-session-report-first-reuse-review-list">
            {sessionFirstQuestionReuseReviewTemplate.items.map(item => (
              <article key={item.id}>
                <div>
                  <span>{item.label}</span>
                  <strong>{item.prompt}</strong>
        ...
        <Button
          size="small"
          type="primary"
          danger={sessionFirstQuestionReuseReviewTemplate.status === 'repair'}
          icon={sessionFirstQuestionReuseReviewTemplate.status === 'repair' ? <ReloadOutlined /> : <CheckCircleOutlined />}
      ...
      <div
        className={`practice-session-report-first-reuse-review-archive status-${sessionFirstQuestionReuseReviewArchive.status}`}
        aria-label="首题复用复盘归档包"
      >
        <div className="practice-session-report-first-reuse-review-archive-head">
          <div>
            <span>首题复用复盘归档包</span>
            <strong>{sessionFirstQuestionReuseReviewArchive.title}</strong>
            <small>{sessionFirstQuestionReuseReviewArchive.summary}</small>
          </div>
        </div>
        {sessionFirstQuestionReuseReviewArchive.items.length === 0 ? (
          <p>等待首题复用复盘归档。先完成首题复用复盘验收后，系统会整理下一轮输入。</p>
        ) : (
          <div className="practice-session-report-first-reuse-review-archive-list">
            {sessionFirstQuestionReuseReviewArchive.items.map(item => (
              <article key={item.id}>
                <div>
                  <span>{item.label}</span>
                  <strong>{item.source}</strong>
        ...
        <Button
          size="small"
          type="primary"
          danger={sessionFirstQuestionReuseReviewArchive.status === 'repair'}
          icon={sessionFirstQuestionReuseReviewArchive.status === 'repair' ? <ReloadOutlined /> : <CheckCircleOutlined />}
