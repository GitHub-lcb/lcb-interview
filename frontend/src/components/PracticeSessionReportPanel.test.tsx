...
describe('PracticeSessionReportPanel', () => {
  ...
  it('renders first question reuse release gate before the next session round', async () => {
    const user = userEvent.setup()
    const onNavigate = vi.fn()
    ...
    const releaseGate = screen.getByLabelText('首题复用放行门禁')

    expect(within(releaseGate).getByText('首题复用放行门禁')).toBeInTheDocument()
    expect(within(releaseGate).getByText('复用暂缓放行')).toBeInTheDocument()
    expect(within(releaseGate).getByText('分数放行')).toBeInTheDocument()
    expect(within(releaseGate).getByText('证据放行')).toBeInTheDocument()
    expect(within(releaseGate).getByText('阻断放行')).toBeInTheDocument()
    expect(within(releaseGate).getByText('下一题放行')).toBeInTheDocument()
    expect(within(releaseGate).getAllByText('放行检查').length).toBeGreaterThan(0)
    expect(within(releaseGate).getAllByText('处理动作').length).toBeGreaterThan(0)

    await user.click(within(releaseGate).getByRole('button', { name: /回到复用修复/ }))

    expect(onNavigate).toHaveBeenCalledWith('/practice?queue=1,2')
  })

  it('renders first question reuse review template before the next session round', async () => {
    const user = userEvent.setup()
    const onNavigate = vi.fn()
    ...
    const reviewTemplate = screen.getByLabelText('首题复用复盘模板')

    expect(within(reviewTemplate).getByText('首题复用复盘模板')).toBeInTheDocument()
    expect(within(reviewTemplate).getByText('回修复用复盘模板')).toBeInTheDocument()
    expect(within(reviewTemplate).getByText('分数回看')).toBeInTheDocument()
    expect(within(reviewTemplate).getByText('证据命中')).toBeInTheDocument()
    expect(within(reviewTemplate).getByText('阻断回收')).toBeInTheDocument()
    expect(within(reviewTemplate).getByText('下一题回流')).toBeInTheDocument()
    expect(within(reviewTemplate).getAllByText('填写提示').length).toBeGreaterThan(0)
    expect(within(reviewTemplate).getAllByText('验收规则').length).toBeGreaterThan(0)

    await user.click(within(reviewTemplate).getByRole('button', { name: /复盘复用结果/ }))

    expect(onNavigate).toHaveBeenCalledWith('/practice?queue=1,2')
  })

  it('renders first question reuse review acceptance before the next session round', async () => {
    const user = userEvent.setup()
    const onNavigate = vi.fn()
    ...
    const acceptance = screen.getByLabelText('首题复用复盘验收卡')

    expect(within(acceptance).getByText('首题复用复盘验收卡')).toBeInTheDocument()
    expect(within(acceptance).getByText('回修复用复盘待验收')).toBeInTheDocument()
    expect(within(acceptance).getByText('分数验收')).toBeInTheDocument()
    expect(within(acceptance).getByText('证据验收')).toBeInTheDocument()
    expect(within(acceptance).getByText('阻断验收')).toBeInTheDocument()
    expect(within(acceptance).getByText('回流验收')).toBeInTheDocument()
    expect(within(acceptance).getAllByText('通过信号').length).toBeGreaterThan(0)
    expect(within(acceptance).getAllByText('补救动作').length).toBeGreaterThan(0)

    await user.click(within(acceptance).getByRole('button', { name: /验收复用复盘/ }))
  ...
  it('renders first question reuse review handoff acceptance before the next session round', async () => {
    const user = userEvent.setup()
    const onNavigate = vi.fn()
    ...
    const acceptance = screen.getByLabelText('首题复用复盘回流验收卡')

    expect(within(acceptance).getByText('首题复用复盘回流验收卡')).toBeInTheDocument()
    expect(within(acceptance).getByText('回修复用复盘回流待验收')).toBeInTheDocument()
    expect(within(acceptance).getByText('分数可核验')).toBeInTheDocument()
    expect(within(acceptance).getByText('证据可带入')).toBeInTheDocument()
    expect(within(acceptance).getByText('阻断可处置')).toBeInTheDocument()
    expect(within(acceptance).getByText('入口可执行')).toBeInTheDocument()
    expect(within(acceptance).getAllByText('通过信号').length).toBeGreaterThan(0)
    expect(within(acceptance).getAllByText('补救动作').length).toBeGreaterThan(0)

    await user.click(within(acceptance).getByRole('button', { name: /验收复盘回流/ }))

    expect(onNavigate).toHaveBeenCalledWith('/practice?queue=1,2')
  })

  it('renders first question reuse review handoff release gate before the next session round', async () => {
    const user = userEvent.setup()
    const onNavigate = vi.fn()
    ...
    const gate = screen.getByLabelText('首题复用复盘回流放行门禁')

    expect(within(gate).getByText('首题复用复盘回流放行门禁')).toBeInTheDocument()
    expect(within(gate).getByText('回流暂缓放行')).toBeInTheDocument()
    expect(within(gate).getByText('分数放行')).toBeInTheDocument()
    expect(within(gate).getByText('证据放行')).toBeInTheDocument()
    expect(within(gate).getByText('阻断放行')).toBeInTheDocument()
    expect(within(gate).getByText('入口放行')).toBeInTheDocument()
    expect(within(gate).getAllByText('放行规则').length).toBeGreaterThan(0)
    expect(within(gate).getAllByText('处理动作').length).toBeGreaterThan(0)
    expect(within(gate).getAllByText('未放行').length).toBeGreaterThan(0)

    await user.click(within(gate).getByRole('button', { name: /回到回流修复/ }))
